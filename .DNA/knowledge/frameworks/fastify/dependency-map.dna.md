# Dependency map — Fastify

## Upstream
List services **Fastify** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **Fastify** is down? (UI routes, jobs, webhooks, partner APIs)
