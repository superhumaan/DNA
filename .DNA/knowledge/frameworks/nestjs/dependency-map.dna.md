# Dependency map — NestJS

## Upstream
List services **NestJS** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **NestJS** is down? (UI routes, jobs, webhooks, partner APIs)
