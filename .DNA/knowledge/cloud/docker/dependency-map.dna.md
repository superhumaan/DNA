# Dependency map — Docker

## Upstream
List services **Docker** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **Docker** is down? (UI routes, jobs, webhooks, partner APIs)
