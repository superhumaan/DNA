# Dependency map — OpenAI

## Upstream
List services **OpenAI** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **OpenAI** is down? (UI routes, jobs, webhooks, partner APIs)
