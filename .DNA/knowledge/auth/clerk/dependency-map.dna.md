# Dependency map — Clerk

## Upstream
List services **Clerk** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **Clerk** is down? (UI routes, jobs, webhooks, partner APIs)
