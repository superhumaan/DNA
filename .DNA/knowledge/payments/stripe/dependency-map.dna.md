# Dependency map — Stripe

## Upstream
List services **Stripe** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **Stripe** is down? (UI routes, jobs, webhooks, partner APIs)
