# Dependency map — MongoDB

## Upstream
List services **MongoDB** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **MongoDB** is down? (UI routes, jobs, webhooks, partner APIs)
