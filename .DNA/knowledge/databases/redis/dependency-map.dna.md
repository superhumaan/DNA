# Dependency map — Redis

## Upstream
List services **Redis** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **Redis** is down? (UI routes, jobs, webhooks, partner APIs)
