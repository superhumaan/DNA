# Dependency map — Drizzle

## Upstream
List services **Drizzle** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **Drizzle** is down? (UI routes, jobs, webhooks, partner APIs)
