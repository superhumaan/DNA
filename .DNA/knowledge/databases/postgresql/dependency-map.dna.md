# Dependency map — PostgreSQL

## Upstream
List services **PostgreSQL** calls (DB, auth, payments, AI, queues). Note timeout and retry policy per dependency.

## Downstream
Who breaks if **PostgreSQL** is down? (UI routes, jobs, webhooks, partner APIs)
