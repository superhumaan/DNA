...
* Docker is the deployable artifact for this repository.

Follow-up verification found this monorepo has no deployable preview root.
Preview is now explicitly disabled in `.DNA/config.dna.json`; branch CI and
Docker are the supported delivery gates. The generator removes stale preview
workflows when opted out.
