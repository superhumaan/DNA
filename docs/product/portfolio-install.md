...
Type `/` in Cursor — **59 stems** and **`/dna-*` commands** are already there.
...
| **`stems/`** (**59 packs**) | Full prompt engineering library — guidelines, expectations, examples, workflows per stem |
...
### 4. Prompt stem packs (59) + slash commands
...
│   └── commands/                     ← 59 stems + /dna-* slash commands
...
| Custom AI rules + prompt library | Consultant / staff time | 59 stems + rules + skills — installed |
...
| **DNA — one `doctor`** | **Yes** | **59 + rules** | **Yes** | **Yes** | **Yes** | **Yes** |
