...
Type `/` in Cursor — **103 stems** and **`/dna-*` commands** are already there.
...
| **`stems/`** (**103 packs**) | Full prompt engineering library — guidelines, expectations, examples, workflows per stem |
...
### 4. Prompt stem packs (103) + slash commands
...
│   └── commands/                     ← 103 stems + /dna-* slash commands
...
| Custom AI rules + prompt library | Consultant / staff time | 103 stems + rules + skills — installed |
...
| **DNA — one `doctor`** | **Yes** | **103 + rules** | **Yes** | **Yes** | **Yes** | **Yes** |
