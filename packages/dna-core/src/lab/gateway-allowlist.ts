import {
  ...
  DNA_LAB_GATEWAY_PUBLIC_PATHS,
} from "@superhumaan/dna-config";

/** Relative paths under the Lab API that must bypass auth gateways for CLI pairing. */
...
export function formatGatewayAllowlistFailure(productionUrl: string, pushError?: string): string {
  ...
  return [
    "FATAL — production was not pre-notified. Pairing was NOT saved on the server.",
    pushError ?? "CLI could not reach POST /api/dna/labs/pairing/init",
    "",
    "Store-first pairing requires an unauthenticated allowlist for:",
    ...DNA_LAB_GATEWAY_PUBLIC_PATHS.map((p) => `  • ${p}`),
    "",
    "Fix (Invitrace Connect / nginx / oauth2-proxy): add those paths to the public bypass list,",
    "restart the gateway, then re-run:",
    `  npx dna register lab --url ${base}`,
    "",
    `See ${DNA_LAB_GATEWAY_ALLOWLIST_FILE} after dna doctor / lab install.`,
    `Probe: curl -sS -o /dev/null -w "%{http_code}\\n" -X POST ${base}/api/dna/labs/pairing/init \\`,
    `  -H "Content-Type: application/json" \\`,
    `  -d '{"pairingId":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","codeHash":"${"b".repeat(64)}","projectId":"app"}'`,
    "Expected: 200 or 400 JSON from DNA — NOT 302 to /_connect/login.",
  ].join("\n");
}

export const GATEWAY_PUBLIC_PATHS_MARKDOWN = `# DNA Lab — gateway public paths (pairing)
